...
class NESROM(object):
    ...
    def to_file(self, file):
        ...
        header[2] = ord('S')
        header[3] = 0x1a
        header[4] = len(self.prom) >> 14
        header[5] = len(self.crom) >> 13
        header[6] = MIRRORING_FOUR_SCREEN if (self.mirroring & MIRRORING_FOUR_SCREEN) else self.mirroring
        header[6] |= (self.ines_mapper & 0x0f) << 4
        header[7] = (self.ines_mapper & 0xf0)

        file.write(header)
        file.write(self.prom)
        file.write(self.crom)